...
<%@ include file="/WEB-INF/jsp/admin/inc/meta.jsp"%>
  ...
  <div class="title_0">您的位置：内容管理 &gt;&gt; 栏目管理</div>

<div class="blank12"></div>
<div align="left">    
  <input class="button_0" name="" type="button" value="添加栏目" onclick="location.href='${approot}${rooturi}/toAdd'"/>  
...
<c:forEach var="item" items="${page.items}">
 ...
 <td>${item.name}</td>
  <td>
  <c:choose>
  <c:when test="${!empty item.seoTitle}"> 
  ${item.seoTitle}
  </c:when>
  ...
  <td>
  <c:choose>
  <c:when test="${!empty item.seoKeyword}"> 
  ${item.seoKeyword}
  </c:when>
  <c:otherwise>无</c:otherwise>
  </c:choose>
  ...
  <td>
  <c:choose>
  <c:when test="${!empty item.seoDesc}"> 
  ${item.seoDesc}
  </c:when>
  <c:otherwise>无</c:otherwise>
  </c:choose>
  ...
  <td>
  <c:choose>
  <c:when test="${item.isShowInNav=='1'}"> 
  <img src="${approot}/static/images/common/no.gif"/> 
  </c:when>
  <c:when test="${item.isShowInNav=='0'}"> 
  <img src="${approot}/static/images/common/yes.gif"/>
   </c:when>
  <c:otherwise>未知</c:otherwise>
  </c:choose>
